        ...
        <tbody>
            ...
            if ($media_length > 0) {
                while ($row = $result->fetch_array()) {
                    ...
                    echo ('    <td class="media-room-item mediaa-room-title">' . RemoveXSS($row['title']) . '</td>');
                    echo ('    <td class="media-room-item mediaa-room-link"><a href="' . RemoveXSS($row['link']) . '" target="_blank" >' . RemoveXSS($row['link']) . '</a></td>');
