...
<div class="mt-4 mb-4 container">
    <div class="m-4 d-flex justify-content-end">
        <div class="row"><button type="button" class="btn btn-info" onclick="javascript: location.href='./press_release_form.php?seq=0';">글쓰기</button></div>
    </div>
    ...
    <table class="table table-hover table-hover-pointer">
        ...
        <tbody>
            ...
            $sql  = "SELECT seq, title, view_count, created_at FROM artgg_press_release WHERE deleted_at IS NULL ORDER BY seq desc LIMIT " . $paging_info['page_db'] . ", $item_row_count";
            $result = mysqli_query($conn, $sql) or exit(mysqli_error($conn));
            $notice_length = $result->num_rows;

            if ($notice_length > 0) {
                while ($row = $result->fetch_array()) {
                    echo ('<tr onclick=getPressReleaseInfo(' . RemoveXSS($row['seq']) . ')>');
                    echo ('    <th scope="row">' . RemoveXSS($row['seq']) . '</th>');
                    echo ('    <td class="press_release-title">' . RemoveXSS($row['title']) . '</td>');
                    echo ('    <td>' . $row['created_at'] . '</td>');
                    echo ('    <td>' . RemoveXSS($row['view_count']) . '</td>');
                    echo ('</tr>');
                }
            } else {
                echo ('<tr><th scope="row"></th><td>등록된 보도자료가 없습니다.</td><td></td><td></td></tr>');
            }
